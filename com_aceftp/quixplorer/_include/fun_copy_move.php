...
Comment:
	...
	Have Fun...
------------------------------------------------------------------------------*/
require_once(JPATH_ROOT . "/" . JPATH_QUIX . "/_include/permissions.php");
//------------------------------------------------------------------------------
...
function dir_print($dir_list, $new_dir) {	// print list of directories
	...
	while(list($new_item,) = each($dir_list)) {
		...
		echo "<TR><TD><A HREF=\"javascript:NewDir('".get_rel_item($new_dir,$new_item).
			...
			"src=\"".JPATH_QUIX_SHORT."/_img/dir.gif\" ALT=\"\">&nbsp;".$s_item."</A></TD></TR>\n";
	}
}
//------------------------------------------------------------------------------
	// copy/move file/dir
function copy_move_items ($dir)
{
	...
	// Copy or Move?
	if($GLOBALS["action"]!="move") {
		$_img=JPATH_QUIX_SHORT."/_img/__copy.gif";
	} else {
		$_img=JPATH_QUIX_SHORT."/_img/__cut.gif";
	}
...
//-->
		...
		echo "<IMG SRC=\"".JPATH_QUIX_SHORT."/_img/__paste.gif\" align=\"ABSMIDDLE\" ALT=\"\">\n";
		...
		for($i=0;$i<$cnt;++$i) {
			...
			echo "<TR><TD><IMG SRC=\"".JPATH_QUIX_SHORT."/_img/_info.gif\" align=\"ABSMIDDLE\" ALT=\"\">";
			// Old Name
